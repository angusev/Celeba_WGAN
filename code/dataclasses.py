...
class ImageDataset(Dataset):
    ...
    def __init__(self, files: List[str], conditions: np.array, id2idx: Dict[str, int], transform: Transform):
        self._files = files
        self._id2idx = id2idx
...
def get_dataloaders(paths, training_configs, launch_configs):
    ...
    with open('Anno/list_attr_celeba.txt') as f:
        ...
        attrs =  torch.cuda.FloatTensor(attrs.drop(columns=['id']).values.astype(float)).to(training_configs.device)
